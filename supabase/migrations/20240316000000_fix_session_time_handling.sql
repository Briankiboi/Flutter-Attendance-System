-- Fix session time handling and UTC conversion
DO $OUTER$ 
BEGIN
    ...
    BEGIN
        ...
        -- Combine date and time into timestamp and set it to Africa/Nairobi timezone
        -- This assumes the input times are in local time (Nairobi)
        v_timestamp := ((v_date::TEXT || ' ' || v_time::TEXT)::TIMESTAMP AT TIME ZONE 'Africa/Nairobi') AT TIME ZONE 'UTC';
...
-- Verify the results with timezone information
SELECT 
    id,
    start_time AT TIME ZONE 'UTC' AS start_time_utc,
    start_time AT TIME ZONE 'Africa/Nairobi' AS start_time_local,
    end_time AT TIME ZONE 'UTC' AS end_time_utc,
    end_time AT TIME ZONE 'Africa/Nairobi' AS end_time_local,
    (NOW() AT TIME ZONE 'UTC') AS current_time_utc,
    (NOW() AT TIME ZONE 'Africa/Nairobi') AS current_time_local,
    is_active,
