...
-- Modify lecturers table to include all necessary fields
ALTER TABLE lecturers ADD COLUMN IF NOT EXISTS occupation TEXT;
ALTER TABLE lecturers ADD COLUMN IF NOT EXISTS employment_type TEXT;
ALTER TABLE lecturers ADD COLUMN IF NOT EXISTS gender TEXT;
ALTER TABLE lecturers ADD COLUMN IF NOT EXISTS profile_image_path TEXT;
ALTER TABLE lecturers ADD COLUMN IF NOT EXISTS lecture_number TEXT;

-- Create lecturer_assigned_units if not exists
...
-- Create the lecturer signup function with proper error handling
CREATE OR REPLACE FUNCTION handle_lecturer_signup()
RETURNS TRIGGER AS $$
DECLARE
  v_department_id UUID;
  v_lecture_number TEXT;
BEGIN
  ...
  -- Generate unique lecture number
  SELECT 'LC' || SUBSTRING(MD5(NEW.id::text) FROM 1 FOR 6) INTO v_lecture_number;
